...
# Tạo script MySQL để tạo showtime ID 6
$mysqlScript = @"
USE movietickets;
...
$mysqlScript | Out-File -FilePath "auto_create_showtime_6.sql" -Encoding UTF8

Write-Host "📝 Created auto_create_showtime_6.sql" -ForegroundColor Green
...
try {
    # Thử chạy MySQL command
    $mysqlCommand = "mysql -u root -p < auto_create_showtime_6.sql"
    Write-Host "Running: $mysqlCommand" -ForegroundColor Cyan
    
    # Chạy MySQL command
    $result = cmd /c "mysql -u root -p < auto_create_showtime_6.sql 2>&1"
    ...
    Write-Host "mysql -u root -p < auto_create_showtime_6.sql" -ForegroundColor Cyan
}
